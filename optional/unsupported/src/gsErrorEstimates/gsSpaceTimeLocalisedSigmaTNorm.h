/** @file gsSpaceTimeLocalisedSigmaTNorm.h

    @brief Computes the localised space-time norm $(sum_K || w ||^2_{\Gamma_{K,T}} + h_K || grad_x w ||^2_{\Gamma_{K,T}})^{1/2}$.
...
namespace gismo
{
    /** @brief The gsSpaceTimeLocalisedSigmaTNorm class provides the functionality
     * to calculate the norm $(sum_K || w ||^2_{\Gamma_{K,T}} + h_K || grad_x w ||^2_{\Gamma_{K,T}})^{1/2}$ on the
    ...
    */
    ...
    public:
        ...
        T compute(bool storeElWise = false)
        {
            ...
            // TODO: figure out why comparison with m_sides.begin()
