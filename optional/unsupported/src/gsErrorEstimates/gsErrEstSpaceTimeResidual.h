/** @file gsErrEstSpaceTimeResidual.h
    ...
    @brief Residual-type error estimator for the Parabolic problem, inspired
    the indicator suggested by H. Yang and O. Stainbach:
    h^2 * || f - v_t + Delta_x v||^2_K, where K is each element
    ...
    protected:
                         ...
                         T & accumulated)
        {
            ...
            for (index_t k = 0; k < quWeights.size(); ++k) // loop over quadrature nodes
            {
                ...
                sumVolSq += weight * math::pow(h_K * (m_phLaplace(0,0) + m_rhsFctVals(0,k) - m_phdiscSolDer.bottomRows(1)(0,0)), 2);
            }
