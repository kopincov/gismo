...
@brief A class that computes $m^2_d := ||grad v - y||^2_Q, dual part of the functional-type
...
/** \brief Provides computation of the dual term of functional-type element-wise error indicator
...
* where the local estimate \f$ \overline{\rm m}^2_{d, K} \f$ on an element \f$ K \f$ is given by
 \f[
 \overline{\rm m}^2_{eq, K} := \int_K (grad v - y )^2 dxdt
 \f]
