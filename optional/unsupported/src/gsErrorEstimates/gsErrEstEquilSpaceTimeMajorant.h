...
@brief  A class that computes $m^2_eq := || div y + f - v_t ||^2_Q, the equlibrated part of the functional-type
       error estimator for the space-time formulation of the heat problem.
...
/** \brief Provides computation of equilibrium term of fucntional-type element-wise error indicator for the
 * space-time formulation of the heat problem.
*
...
* \f[ \overline{\rm m}^2_eq := \sum_K \overline{\rm m}^2_{eq, K} \f]
* where the local estimate \f$ \overline{\rm m}^2_{eq, K} \f$ on an element \f$ K \f$ is given by
 \f[
 \overline{\rm m}^2_{eq, K} := \int_K ( div y + f - v_t )^2 dxdt
 \f]
...
*/
    ...
    public:
        ...
        */
        ...
        gsErrEstEquilSpaceTimeMajorant(const gsMultiPatch<T> & mp,
                                       const gsField<T> & _approxSol,
                                       const gsField<T> & _approxFlux,
                                       const gsFunction<T> & _rhsFunction,
                                       bool _rhsFunctParam = false)
                : gsEstimator<T>(mp, _approxSol,_approxFlux, _rhsFunction),
                  m_f2param(_rhsFunctParam)
        {
            m_bcInitialized = false;    // since no bc is given
            m_storeElWiseType = 0;      // auxiliary flag, mainly for debugging
    ...
    public:
         ...
         * \f$\overline{\rm m}^2_{eq, K} := \int_K ( div y + f - v_t )^2 dxdt\f$
         *
    ...
    protected:
                        ...
                        unsigned      & evFlags) // replace with geoEval ?
        {
            ...
            // Initialize a number of quadrature nodes for each of the sub-dimenisions based on the degree of a given basis
        ...
        */
                         ...
                         T & maj)
        {
            ...
            //gsInfo << "geoEval.geometry().geoDim() = " <<  geoEval.geometry().geoDim() << "\n";
            //gsInfo << "geoEval.geometry().function(0) = " << geoEval.geometry().function(0) << "\n";
            //gsInfo << "geoEval.geometry().function(0).dimensions() = " << geoEval.geometry().function(0).dimensions() << "\n";
            ...
            //gsInfo << "m_approxGrad.size() = \n " << m_approxGrad.size() << "\n";
            m_phApproxGrad.setZero(m_approxGrad.rows(), m_approxGrad.cols());
            ...
            for (index_t k = 0; k < quWeights.size(); ++k) // loop over quadrature nodes
            {
                ...
                if (weight) {
                    ...
                    // Copy v_t (time component) from grad(v)
                    m_phApproxTimeGrad = m_phApproxGrad.bottomRows(1);
                    //gsInfo << "m_phApproxTimeGrad = \n " << m_phApproxTimeGrad << "\n";

                    // Copy y_x (spacial components) from y (and keeping dth component zero)
