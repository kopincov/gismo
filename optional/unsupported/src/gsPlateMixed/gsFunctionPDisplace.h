...
template<class T>
class gsFunctionPDisplace : public gsFunction<T>
{
public:
    typedef gsFunction<T> Base;

    /// Shared pointer for gsFunctionPDisplace
    typedef memory::shared_ptr< gsFunctionPDisplace > Ptr;

    /// Unique pointer for gsFunctionPDisplace
    typedef memory::unique_ptr< gsFunctionPDisplace > uPtr;
...
public:
    gsFunctionPDisplace(const gsGeometry<T>& geometry,const gsGeometry<T>& u_co):
        m_geometry(geometry), m_u_co(u_co)
    {}


    GISMO_UPTR_FUNCTION_NO_IMPLEMENTATION(gsFunctionPDisplace, clone)
