/** @file gsVisitorShellMixedFree.h
...
template <class T>
class gsVisitorShellMixedFree
{
public:

    gsVisitorShellMixedFree(const gsPde<T> & pde, const boundary_condition<T> & s, const gsShellMixedAssembler<T>& assembler)
    ...
    {
        // Set options
        lambdaTMean0 = options.getSwitch("lambdaTMean0");
                         ...
                         const gsMatrix<T>    & quNodes)
    {
        ...
        */
        ...
        localMatLn0.setZero(numActive[lambdaBasis], numActive[pBasis]);
        localMatLn1.setZero(numActive[lambdaBasis], 2 * numActive[phiBasis]);
        localMatLt0.setZero(numActive[lambdaBasis], numActive[pBasis]);
        localMatLt1.setZero(numActive[lambdaBasis], 2 * numActive[phiBasis]);
                         ...
                         const gsVector<T>   & quWeights)
    {
        ...
        for (index_t k = 0; k < quWeights.rows(); ++k)
        {
            ...
            */
            ...
            for ( index_t i =0; i<numActive[lambdaBasis]; i++)
            {
                ...
                for (index_t j = 0; j < numActive[phiBasis]; j++)
                    for(index_t l=0; l<2; l++)
                    {
                        ...
                        // Ln1
                        localMatLn1(i,l*numActive[phiBasis] + j) += weight * ((jac_j *tangent).dot(unormal) * lambdaNCoef_Ln + (jac_j *tangent).dot(tangent) *lambdaTCoef_Ln ) * bVals[lambdaBasis](i,k);

                        // Lt1
                        localMatLt1(i,l*numActive[phiBasis] + j) += weight * ((jac_j *tangent).dot(unormal) * lambdaNCoef_Lt + (jac_j *tangent).dot(tangent) *lambdaTCoef_Lt ) * bVals[lambdaBasis](i,k);
                    }

                for (index_t j = 0; j < numActive[pBasis]; j++)
                {
                    // Ln0
                    localMatLn0(i,j) += weight * bVals[pBasis](j,k) * lambdaNCoef_Ln * bVals[lambdaBasis](i,k);
                    // Lt0
                    localMatLt0(i,j) += weight * bVals[pBasis](j,k) * lambdaNCoef_Lt * bVals[lambdaBasis](i,k); // only non-zero at free coners, lambdaNCoef_Lt = 0 in interior
                }
            ...
            // Ln0, Ln1 lagrange multiplier p + (grad phi)_tn
            // Ln1
                    ...
                    for(index_t l=0; l<2; l++)
                    {
                        ...
                        if(actives(i,0) == cornerIndex[0])
                        {
                            if(pde.m_corners[m_patchIndex][corners[0]] == gsShellMixedPde<T>::sf)
                                localMatLn1(i,l*numActive + j) += weight * (pde.m_cornerCouplingCoefs[m_patchIndex][corners[0]](3,0)*(jac_j *tangent).dot(unormal) + pde.m_cornerCouplingCoefs[m_patchIndex][corners[0]](1,0)*(jac_j *tangent).dot(tangent) ) * bVals(i,k);
                        ...
                        else
                        {
                            if(actives(i,0) == cornerIndex[1])
                            {
                                if(pde.m_corners[m_patchIndex][corners[1]] == gsShellMixedPde<T>::sf)
                                    localMatLn1(i,l*numActive + j) += weight * (pde.m_cornerCouplingCoefs[m_patchIndex][corners[1]](2,0)*(jac_j *tangent).dot(unormal) + pde.m_cornerCouplingCoefs[m_patchIndex][corners[1]](0,0)*(jac_j *tangent).dot(tangent) ) * bVals(i,k);
            ...
            for ( index_t i =0; i<numActive; i++)
                for (index_t j = 0; j < numActive; j++)
                {
                    ...
                    {
                        if(pde.m_corners[m_patchIndex][corners[0]] == gsShellMixedPde<T>::sf)
                            localMatLn0(i,j) += weight * pde.m_cornerCouplingCoefs[m_patchIndex][corners[0]](3,0) * bVals(j,k) * bVals(i,k);
                        if(pde.m_corners[m_patchIndex][corners[0]] == gsShellMixedPde<T>::none)
                            localMatLn0(i,j) += weight * bVals(j,k) * bVals(i,k);
                    ...
                    else
                    {
                        ...
                        {
                            if(pde.m_corners[m_patchIndex][corners[1]] == gsShellMixedPde<T>::sf)
                                localMatLn0(i,j) += weight * pde.m_cornerCouplingCoefs[m_patchIndex][corners[1]](2,0) * bVals(j,k) * bVals(i,k);
                            if(pde.m_corners[m_patchIndex][corners[1]] == gsShellMixedPde<T>::none)
                                localMatLn0(i,j) += weight * bVals(j,k) * bVals(i,k);
            ...
            // Lt lagrange multiplier (grad phi)_tt
            for ( index_t i =0; i<numActive; i++)
                for (index_t j = 0; j < numActive; j++)
                    for(index_t l=0; l<2; l++)
                    {
                        ...
                        localMatLt(i,l*numActive + j) += weight * (jac_j *tangent).dot(tangent) * bVals(i,k);
                              ...
                              gsSparseSystem<T>         & system)
    {
        ...
        system.mapColIndices(actives[lambdaBasis], patchIndex, actives_vec[10], 10);

        if(lambdaTMean0)
        {
            actives_vec[11].resize(1,1);
            if(pde.m_indFreeComp[m_patchIndex][side] == -1)
            {
                //gsInfo<<"Wrong free index !!";
            }
            else
                actives_vec[11] << pde.m_indFreeComp[m_patchIndex][side];

            //eliminatedDofs[11].resize(1,1);
            //eliminatedDofs[11] << 0;
        }
        ...
        if(lambdaTMean0 && pde.m_indFreeComp[m_patchIndex][side] != -1)
        {
            // LtMean
...
protected:
    ...
    gsMatrix<T> localMatLn0, localMatLn1, localMatLt0, localMatLt1, localMatLnMean, localMatLtMean;
