...
/** @brief The gsH1BoundaryNorm class provides the functionality
...
*/
template <class T>
class gsH1BoundaryNorm : public gsNorm<T>
{
    friend class gsNorm<T>;
        
public:

    gsH1BoundaryNorm(const gsField<T> & _field1,
                     const gsFunction<T> & _func2,
                     boxSide s,
                     bool _f2param = false)
    : gsNorm<T>(_field1,_func2), dfunc2(NULL), f2param(_f2param), side(s)
    { 
    ...
    gsH1BoundaryNorm(const gsField<T> & _field1,
                     ...
                     boxSide s,
                     bool _f2param = false)
    : gsNorm<T>(_field1,_func2), dfunc2(&_dfunc2), f2param(_f2param), side(s)
    { 
        
    }
    
    gsH1BoundaryNorm(const gsField<T> & _field1, boxSide s)
    : gsNorm<T>(_field1), dfunc2(NULL), f2param(false), side(s)
    {
