...
/** @file gsMagnetostaticShapeDerivAssembler_decoupled.h
...
*/
template <class T>
class gsMagnetostaticShapeDerivAssembler_decoupled : public gsAssembler<T>
{
public:
...
public:
    gsMagnetostaticShapeDerivAssembler_decoupled( const gsMagnetostaticShapeDerivPde<T> & pde, const gsMultiBasis<T> & bases,
    ...
    {
        return new gsMagnetostaticShapeDerivAssembler_decoupled<T>(*this);
    }
    ...
    virtual void assemble()
    {
        ...
        //variable e1 = exprAss.getCoeff(unit1, G); // treated as column vectors
        //variable e2 = exprAss.getCoeff(unit2, G);
        ...
        gsExprAssembler<T> exprAss2(1,1);
        exprAss2.setOptions(m_options);
        exprAss2.setIntegrationElements(m_bases.front());

        space phi2 = exprAss2.getSpace(m_bases.front(), 2);
        geometryMap G2 = exprAss2.getMap(domain);
        variable matCoeff = exprAss2.getCoeff(*(ppde->getAlpha()), G2);
        variable magnetization = exprAss2.getCoeff(*(ppde->getRhs()), G2); // method rhs() gives only the 0-th piece of the rhs; works well for IETI
        variable uh = exprAss2.getCoeff(*(ppde->getStateSolution()));
        variable ph = exprAss2.getCoeff(*(ppde->getAdjointSolution()));
        //variable nu = exprAss2.getCoeff(nu_mag, G2);
        exprAss2.getCoeff(nu_mag, G2);
        //variable e1 = exprAss2.getCoeff(unit1, G2); // treated as column vectors
        //variable e2 = exprAss2.getCoeff(unit2, G2);

        phi2.setInterfaceCont(0);
        phi2.addBc((m_pde_ptr->bc()).get("Dirichlet"));
        ...
        // rhs expression
        /*auto rhs_expr1 =    ijac(phi2, G2) % (id(2) * (matCoeff.val() * (fjac(uh).tr() * jac(G2).ginv() * (fjac(ph).tr() * jac(G2).ginv()).tr())).val() ) +
                            ijac(phi2, G2) % (id(2) * ((-fjac(ph).tr()) * jac(G2).ginv() * magnetization / 1.086).val() ) +
                            ijac(phi2, G2) % ((1/1.086) * magnetization * (fjac(ph).tr() * jac(G2).ginv())).tr() +
                            ijac(phi2, G2) % ((-matCoeff.val()) * ( ((jac(G2).ginv()).tr() * fjac(ph)) * (fjac(uh).tr() * jac(G2).ginv()) )) +
                            ijac(phi2, G2) % ((-matCoeff.val()) * ( ((jac(G2).ginv()).tr() * fjac(uh)) * (fjac(ph).tr() * jac(G2).ginv()) ));
        */  // auto isn't allowed in C++98,

        /*auto rhs_expr2 =    (igrad(phi, G) * e2) % (id(2) * (matCoeff.val() * (fjac(uh).tr() * jac(G).ginv() * (fjac(ph).tr() * jac(G).ginv()).tr())).val() ) +
                            (igrad(phi, G) * e2) % (id(2) * ((-fjac(ph).tr()) * jac(G).ginv() * magnetization / 1.086).val() ) +
                            (igrad(phi, G) * e2) % ((1/1.086) * magnetization * (fjac(ph).tr() * jac(G).ginv())).tr() +
                            (igrad(phi, G) * e2) % ((-matCoeff.val()) * ( ((jac(G).ginv()).tr() * fjac(ph)) * (fjac(uh).tr() * jac(G).ginv()) )) +
                            (igrad(phi, G) * e2) % ((-matCoeff.val()) * ( ((jac(G).ginv()).tr() * fjac(uh)) * (fjac(ph).tr() * jac(G).ginv()) ));
        */

        // Assemble the lhs and rhs, lhs depends on the choice of the Hilbert space
        //exprAss.assemble( ((phi * phi.tr()) + (scale.val() * (igrad(phi, G) * igrad(phi, G).tr())) ) * meas(G), rhs_expr1 * meas(G),  rhs_expr2 * meas(G));
        exprAss.assemble( ((phi * phi.tr()) + (scale.val() * (igrad(phi, G) * igrad(phi, G).tr())) ) * meas(G));
        m_system.matrix() = exprAss.matrix();

        // Wrong scaling!!!
        /*exprAss2.assemble(
            ijac(phi2, G2) % (id(2) * (matCoeff.val() * (fjac(uh).tr() * jac(G2).ginv() * (fjac(ph).tr() * jac(G2).ginv()).tr())).val() ) +
            ijac(phi2, G2) % (id(2) * ((-fjac(ph).tr()) * jac(G2).ginv() * magnetization / 1.086).val() ) +
            ijac(phi2, G2) % ((1/1.086) * magnetization * (fjac(ph).tr() * jac(G2).ginv())).tr() +
            ijac(phi2, G2) % ((-matCoeff.val()) * ( ((jac(G2).ginv()).tr() * fjac(ph)) * (fjac(uh).tr() * jac(G2).ginv()) )) +
            ijac(phi2, G2) % ((-matCoeff.val()) * ( ((jac(G2).ginv()).tr() * fjac(uh)) * (fjac(ph).tr() * jac(G2).ginv()) ))
        ...
        exprAss2.assemble(
            ijac(phi2, G2) % (id(2) * (matCoeff.val() * (fjac(uh).tr() * jac(G2).ginv() * (fjac(ph).tr() * jac(G2).ginv()).tr())).val() ) +
            ijac(phi2, G2) % (id(2) * ( ((-fjac(ph).tr()) * jac(G2).ginv() * (magnetization))).val() ) +
            ijac(phi2, G2) % ((fjac(ph).tr() * jac(G2).ginv()).tr() * (magnetization).tr()) +
            ijac(phi2, G2) % ((-matCoeff.val()) * ( ((jac(G2).ginv()).tr() * fjac(ph)) * (fjac(uh).tr() * jac(G2).ginv()) )) +
            ijac(phi2, G2) % ((-matCoeff.val()) * ( ((jac(G2).ginv()).tr() * fjac(uh)) * (fjac(ph).tr() * jac(G2).ginv()) ))
                * meas(G2));

        int size = exprAss2.rhs().rows();
        gsVector<T> rhs(size), rhs2(size/2);
        rhs = exprAss2.rhs();

        m_system.rhs().col(0) = rhs.head(size/2);
        m_system.rhs().col(1) = rhs.tail(size/2);
