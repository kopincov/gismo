...
void gsPdeAssembler<T>::reconstructSolution(bool hasMatrixRhs)
{
    if (this->m_patches.nPatches() == 1)     // single-patch case
    {
        for (signed k=0; k< m_unknownDim.size(); ++k )
        {
            this->m_solutions.push_back(
                new gsField<T>(this->m_patches,
                typename gsFunctionSet<T>::Ptr(reconstructPatchSolution(k,0, hasMatrixRhs)), true ));
        }
    }
    else        // multipatch case
    {
        ...
        for (signed k=0; k< m_unknownDim.size(); ++k ) // For each unknown
        {
            ...
            {
                sols->addPiecePointer( reconstructPatchSolution(k,np, hasMatrixRhs) );
            }
            this->m_solutions.push_back(new gsField<T>(mp, typename gsFunctionSet<T>::Ptr(sols), true ));
        }
    }
}
...
gsFunction<T> * gsPdeAssembler<T>::reconstructPatchSolution(index_t unk, int p, bool hasMatrixRhs) const
{
    ...
    const short_t targetDim = m_unknownDim[unk];
...
void gsPdeAssembler<T>::computeDirichletDofs(dirichlet::strategy dirStrategy)
{
    ...
    for (size_t k=0; k< m_fixedDofs.size(); ++k)
    {
        int basisBmaxsz = 0;
    ...
    for (unsigned k=0; k< m_fixedDofs.size(); ++k)
    {
                  ...
                  it = m_bconditions[k]->dirichletBegin(); it != m_bconditions[k]->dirichletEnd(); ++it )
        {
            ...
            if (bases_sz == m_unknownDim.size())
                ...
                gsMatrix<index_t> boundary = m_bases[k][it->patch()].boundary(it->side()) ;
                ...
                typename gsBasis<T>::uPtr h = m_bases[k][it->patch()].boundaryBasis(it->side());
            ...
            else if (bases_sz == m_unknownDim.sum())
            {
                ...
                {
                    // Get DoFs on this boundary
                    gsMatrix<index_t> boundary = m_bases[comp+component_count][it->patch()].boundary(it->side()) ;
                    ...
                    // Interpolate Dirichlet boundary
                    typename gsBasis<T>::uPtr h = m_bases[comp+component_count][it->patch()].boundaryBasis(it->side());
                                 ...
                                 gsVector<index_t> blockMapper)
{
    ...
    for (index_t comp = 0; comp < tarDim; ++comp)
    {
        ...
        typename gsGeometryEvaluator<T>::uPtr geoEval(getEvaluator(NEED_VALUE |NEED_JACOBIAN, m_patches.patch(patchIndex)) );
             ...
             domIter->good(); domIter->next())
        {
            ...
            mapper[comp]->localToGlobal( bdata.actives, patchIndex, bdata.actives);
            const index_t numActive = bdata.actives.rows();
            B[comp].compute(quNodes, bdata);
            ...
            for (index_t k=0; k!= bdata.allValues().cols(); ++k) // For all quadrature points
            {
                ...
                localRhs.col(0).noalias() += fff * bdata.values[0].col(k);
            }
            ...
            else
            {
                for (index_t j=0; j!=numActive; ++j)
                {
                    ...
                    const unsigned jj = bdata.actives[j];
                    if (mapper[comp]->is_free_index(jj))
                    {
                                 ...
                                 const T kappa)
{
    ...
    for (index_t comp = 0; comp < tDim; ++comp)
    {
        ...
        typename gsGeometryEvaluator<T>::uPtr
            geoEval (getEvaluator(NEED_VALUE | NEED_JACOBIAN | NEED_GRAD_TRANSFORM, m_patches.patch(patchIndex)));
        ...
        // Compute the quadrature rule (nodes and weights)
        quRule = gsQuadRule<T>(bd_intNodes);
        quRule.mapTo(domIter->lowerCorner(), domIter->upperCorner(), quNodes, quWeights);
        ...
        for (; domIter->good(); domIter->next())
        {
            ...
            B[comp]->evalAllDers_into(quNodes, 1, basisData);
            ...
            for (index_t k=0; k!= bdata.allValues().cols(); ++k) // For all quadrature points
            {
                ...
                const T fff = kappa * quWeights[k] * unormal.norm();
