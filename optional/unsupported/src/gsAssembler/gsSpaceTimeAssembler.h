...
#include <gsAssembler/gsVisitorSpaceTime.h>
#include <gsAssembler/gsVisitorDgSpaceTime.h>
#include <gsAssembler/gsVisitorNeumann.h> 
#include <gsAssembler/gsH1BoundaryNorm.h>
...
*/
    ...
    void assemble()
    {
        ...
        Base::template push<gsVisitorSpaceTime<T> >();
        ...
        const T pp = this->penalty(it->first().patch);
        gsVisitorDgSpaceTime<T> dgspacetime(pp, it->first().side());
        this->apply(dgspacetime, iFace);
        }
        }
        */
