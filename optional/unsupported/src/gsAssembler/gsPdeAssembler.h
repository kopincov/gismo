...
public:
    ...
    gsGeometry<T> & patch(size_t i) const {return m_patches.patch(i); }

    /// Return the solution field for component \em i.
    const gsField<T> & solution(size_t i = 0) const   {return *m_solutions[i]; }
    ...
    const gsDofMapper& dofMapper(size_t i = 0) const     { return *m_dofMapper[i]; }
    ...
    short_t fieldDim(size_t i = 0) { return this->m_unknownDim[i]; }
    ...
    const gsMultiBasis<T> & basis(size_t i = 0) const {return m_bases[i];}
