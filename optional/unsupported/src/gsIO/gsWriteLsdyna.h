...
#include <gsIO/gsLsdynaProblems.h>
#include <gsIO/gsLsdynaGeometry.h>
...
// Base class for writing, it has some common features.
class gsLsdynaWriter
{
    // --------------------------------------------------
    // constructors - destructors
    // --------------------------------------------------
public:
    gsLsdynaWriter(lsdyna::lsInt mem = 300)
        :
        memory(mem)
    {
...
public:    
    ...
    {
        lsdyna::write::KEYWORD(out, memory);
        lsdyna::write::TITLE(out, title);
    }
    ...
    void setMemorySize(const lsdyna::lsInt size)
    {
        memory = size;
    }
...
private:
    ...
    gsLsdynaWriter(const gsLsdynaWriter& other);
    const gsLsdynaWriter operator=(const gsLsdynaWriter& other);
...
private:
    ...
    // The memory size to be used in words.
    lsdyna::lsInt memory;
};
...
template <typename T>    
class gsLsdynaFEM: public gsLsdynaWriter
{
public:
    ...
    gsLsdynaFEM(const gsGeometry<T>& geom)
        :
        gsLsdynaWriter(),
        mGeom(geom)
    {
...
public:
    ...
    void writeKFile(std::ostream& out,
                    const std::string& title,
                    const gsVector<index_t, 2>& gridDimension) const
    {
        writeHeader(out, title);

        lsdyna::gsLsdynaEigProblem problem;
        problem.write(out);

        lsdyna::gsLsdynaFEMShell<T> shell(mGeom);
        shell.write(out, gridDimension);
...
template <typename T>
class gsLsdynaIGA : public gsLsdynaWriter
{
public:
    ...
    gsLsdynaIGA(const gsGeometry<T>& geom)
        :
        gsLsdynaWriter(),
        mGeom(geom)
    {
...
public:
                    ...
                    const std::string& title) const
    {
        ...
        lsdyna::gsLsdynaIGAShell<T> shell(mGeom);
        shell.write(out);
...
// ======================================================================
//
// below is old code which should be refactored
//    
// ======================================================================
// ======================================================================    
...
private:
    ...
    {
        pGeoEval = getEvaluator(NEED_VALUE | NEED_MEASURE, mSurf);
        ...
        quRule = gsGaussRule<T>(cwiseIntPoints);
        bdata = gsFuncData<T>(NEED_DERIV|NEED_ACTIVE|SAME_ELEMENT);
        ...
        gsGaussRule<T> quRuleThick(thickIntPoints);
        gsMatrix<T> quNodesThick;
        gsVector<T> quWeightsThick;
        gsFuncData<T> bdataThick(NEED_DERIV|NEED_ACTIVE|SAME_ELEMENT);
        ...
        for(; pDomIt->good(); pDomIt->next())
        {
            ...
            quRuleThick.mapTo(pThickDomIt->lowerCorner(), pThickDomIt->upperCorner(), quNodesThick, quWeightsThick);
            mThickBasis->compute(quNodesThick, bdataThick);

            // normal integration
            quRule.mapTo(pDomIt->lowerCorner(), pDomIt->upperCorner(), quNodes, quWeights);
            mBasis.compute(quNodes, bdata);

            pGeoEval->evaluateAt(quNodes);
            
            defineElementGeneralizedSolid(out, newCoefs, bdataThick, quWeightsThick);

            elementGeneralizedSolid(out, bdataThick);
            
            int saveNodeId2 = mNodeId2;
            constrainedNodeInterpolation(out, newCoefs, pThickDomIt);
            
            elementInterpolationSolid(out, newCoefs, saveNodeId2, pThickDomIt, bdataThick, quNodesThick, quWeightsThick);
    ...
    void elementInterpolationSolid(std::ostream& out,
                                   ...
                                   gsDomainIterator<T>* pThickDomIt,
                                   const gsFuncData<T> & bdataThick,
                                   const gsMatrix<T> & quNodesThick,
                                   const gsVector<T> & quWeightsThick)
    {
        const gsMatrix<index_t>& actFunctions = bdata.actives;
        const index_t numAct = actFunctions.rows();

        const index_t thNumAct = bdataThick.actives.rows();
        ...
        const index_t glNumIntPts = bdata.allValues().cols() * bdataThick.allValues().cols();
        gsMatrix<T> M, G;
        ...
        getGrads(glGrads, newCoefs, pThickDomIt, bdataThick, quNodesThick, quWeightsThick);
        
        for (index_t k = 0; k != bdata.allValues().cols(); ++k)
        {
            for (index_t thK = 0; thK != bdataThick.allValues().cols(); ++thK)
            {
                const T qWeight = quWeights[k] * quWeightsThick[thK]
                    * glGrads.block(0, 3 * (3 * k + thK), 3, 3).determinant();
                
                const gsVector<T>& vals = bdata.values[0].col(k);
                const gsVector<T>& thVals = bdataThick.values[0].col(thK);
                gsVector<T> values2(vals.rows() * thVals.rows());
                for (index_t r = 0; r != thVals.rows(); r++)
                {
                    values2.block(r * vals.rows(), 0, vals.rows(), 1) 
                        = vals * thVals(r);
                }

                M.noalias() += qWeight * (values2 * values2.transpose());
                G.col(k * bdataThick.allValues().cols() + thK) = qWeight * values2;
            }
        }
                  ...
                  const gsVector<T> & quWeightsThick)
    {
        ...
        glGrads.setZero(3, bdata.allValues().cols() * bdataThick.allValues().cols() * 3);
        glGrads.setZero();

        gsMatrix<T> grad , thGrad, value, thValue;

        for (index_t par = 0; par != bdata.allValues().cols(); par++)
        {
            for (index_t act = 0; act != numAct; act++)
            {
                unsigned index = actFunctions(act, 0);
                mBasis.evalSingle_into(index, quNodes.col(par), value);
                mBasis.derivSingle_into(index, quNodes.col(par), grad);

                for (index_t thAct = 0; thAct != thNumAct; thAct++)
                {
                    for (index_t thPar = 0; 
                         thPar != bdataThick.allValues().cols();
                         thPar++)
                    {
                        const gsMatrix<T>& p = quNodesThick.col(thPar);
                        mThickBasis->evalSingle_into(thAct, p, thValue);
                        mThickBasis->derivSingle_into(thAct, p, thGrad);
                        
                        const index_t parameter = 
                            par * bdataThick.allValues().cols() + thPar;
    ...
    void constrainedNodeInterpolation(std::ostream& out,
                                      ...
                                      gsDomainIterator<T>* pThickDomIt)
    {
        gsMatrix<index_t> actFunctions = bdata.actives;
        gsMatrix<T> newNodes, params, thParams, values, thValues;
        getInterpolationNodes(newCoefs, pThickDomIt, params, thParams, 
                              values, thValues, newNodes); 
                                 ...
                                 const gsFuncData<T> & bdataThick)
    {
        ...
        const index_t numAct = actFunctions.rows();
        const index_t numThickAct = bdataThick.actives.rows();
    ...
    void defineElementGeneralizedSolid(std::ostream& out,
                                       ...
                                       const gsVector<T> & quWeightsThick)
    {
        const index_t numAct = bdata.actives.rows();
        const index_t numThickAct = bdataThick.actives.rows();

        out << "*DEFINE_ELEMENT_GENERALIZED_SOLID\n"
            << std::setw(10) << mFormId 
            << std::setw(10) << bdata.allValues().cols() * bdataThick.allValues().cols()
            << std::setw(10) << numAct  * numThickAct 
            << std::setw(10) << 0 << "\n";
        
        for (index_t k = 0; k < bdata.allValues().cols(); ++k)
        {
            gsMatrix<T> derivs = bdata.values[1].col(k);
            derivs.resize(2, numAct);
            
            const gsVector<T>& values = bdata.values[0].col(k);

            for (index_t thickK = 0; thickK < bdataThick.allValues().cols(); ++thickK)
            {
                gsVector<T> thickDerivs = bdataThick.values[1].col(thickK);
                const gsVector<T>& thickValues = 
                    bdataThick.values[0].col(thickK);
                
                index_t gaussPointNumber =
                    bdataThick.allValues().cols() * k + thickK + 1;

                out << "$ gauss point " << gaussPointNumber << "\n";            
                
                out << std::setprecision(13);
                out << std::setw(20) 
                    << quWeights[k] * quWeightsThick[thickK]
                    << "\n";
    ...
    void writeNodes(std::ostream& out,
                    const gsMatrix<T>& newCoefs)
    {
        gsMatrix<index_t> pBoundary = mBasis.allBoundary();
        ...
        const index_t nmbOfSurfaceCoefs = static_cast<index_t>(mBasis.size());
        const int nmbOfThickCoefs = mThickBasis->size();
        for (index_t i = 0; i < nmbOfSurfaceCoefs; ++i)
        {
                               ...
                               gsMatrix<T>& newCoefs)
    {
        ...
        gsMatrix<> deriv;
        mSurf.jacobian_into(grevile, deriv);
    ...
    {
        pGeoEval = getEvaluator(NEED_JACOBIAN | NEED_VALUE | NEED_MEASURE 
                                   | NEED_GRAD_TRANSFORM, mSurf);
        pDomIt = mBasis.makeDomainIterator().release();
                                   ...
                                   const int saveNodeId2)
    {
        ...
        gsMatrix<T> M, G(numAct, bdata.allValues().cols());
        ...
        for (index_t k = 0; k < bdata.allValues().cols(); ++k) // loop over quadrature nodes
        {
            ...
            // const T qweight1 = quWeights[k] * pGeoEval->measure(k);
...
/*            const gsMatrix<T>& coefs = mSurf.coefs();
            ...
            const T qweight = quWeights[k] * Jg.determinant();
            // std::cout << "qweight1: " << qweight1 << "\n"
            //           << "qweight: " << qweight << std::endl;
            

            M.noalias() += qweight * (bdata.values[0].col(k) *
                                       bdata.values[0].col(k).transpose());

            G.col(k) = qweight * bdata.values[0].col(k) ;
        }
        ...
        gsMatrix<T> nodeWeights(1, bdata.allValues().cols());
        ...
        do
        {
            ...
            out << "*ELEMENT_SHELL\n"
                ...
                std::setw(8) << mNodesMap[saveNodeId2 + id + str] << "\n"
                 ...
                 << std::setw(10) << mElementShellId << 
                std::setw(10) << mElementId <<
                std::setw(10) << bdata.allValues().cols() << "\n";
            
            int counter = 1;
            for (index_t w = 0; w < bdata.allValues().cols(); ++w)
            {
                out << std::setw(10) << w + 1 <<
                    std::setw(10) << nodeWeights(0, w);
    ...
    void defineElementGeneralizedShell(std::ostream& out)
    {
        ...
        out << "*DEFINE_ELEMENT_GENERALIZED_SHELL\n"
             << std::setw(10) << mFormId << std::setw(10) << bdata.allValues().cols()
    ...
    void writeNodalDifferentialValues(std::ostream& out)
    {
        ...
        const gsMatrix<T>& A = bdata.values[0].transpose();
        // A -- rows = quadrature points
        //   -- cols = active functions
        
        const gsMatrix<T>& B = bdata.values[1].transpose();
    ...
    void writeNodalDifferentialValues2(std::ostream& out)
    {
        ...
        if (numAct > dimension) // too many functions active, do the extrapolation
        {
            ...
            gsMatrix<T> F(bdata.allValues().cols(),dimension); // evaluation of the monomials in the Gauss points (rows = Gauss points, cols = monomials)

            for (index_t k = 0; k != bdata.allValues().cols(); ++k)
            {
                const gsVector<T> pt = quNodes.col(k);
                int col = 0;
                ...
                for (index_t i = 0; i < mBasis.degree(0) + 1; ++i)
                {
                    ...
                    {
                        F( k,col ) = math::pow(pt[0],i) * math::pow(pt[1],j);
                        col++;
                    }
                }
            }
            ...
            for (index_t i = 0; i != numAct; ++i)
            {
                FFup.col(actFunctions.at(i)) = A.col(i);
            }
            ...
            {
                const real_t x = points(0,k);
                const real_t y = points(1,k);
                ...
                for (index_t i = 0; i < mBasis.degree(0) + 1; ++i)
                {
                    for (index_t j = 0; j < mBasis.degree(1) + 1; ++j)
                    {
                        Frhs( k, col ) = math::pow(x,i) * math::pow(y,j);
                        col++;
                    }
                }
            }
            ...
            for (index_t i = 0; i != numAct; ++i)
            {
                nodalValues.row(i) = thbCoefs.row(actFunctions.at(i));
            }
