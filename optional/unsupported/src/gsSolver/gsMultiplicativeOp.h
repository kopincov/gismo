...
public:
    ...
    gsMultiplicativeOp(memory::shared_ptr< gsSparseMatrix<T> > underlying, TransferContainer transfers, OpContainer ops)
        : m_underlying(give(underlying)), m_transfers(give(transfers)), m_ops(give(ops)), m_upds(m_ops.size()),
          m_res(m_ops.size()), m_p(m_ops.size())
    {
...
#endif
        for (size_t i=0; i<sz; ++i)
            m_upds.push_back(computeUpdMatrix(transfers[i]));
    }
    ...
    void addOperator(Transfer transfer, OpPtr op)
    {
        GISMO_ASSERT ( transfer.rows()==m_underlying->rows()
                       ...
                       <<transfer.rows()<<"=="<<m_underlying->rows()
                       <<"&&"<<transfer.cols()<<"=="<<op->rows()
                       <<"&&"<<op->cols()<<"=="<<op->rows() );
        m_upds.push_back(computeUpdMatrix(transfer));
        m_transfers.push_back(give(transfer));
        m_ops.push_back(give(op));
        m_res.push_back(gsMatrix<T>());
        m_p.push_back(gsMatrix<T>());
    }
...
private:
    ...
    Transfer computeUpdMatrix( const Transfer& transfer )
    {
        return transfer.transpose() * (*m_underlying);
    }

protected:
    memory::shared_ptr< gsSparseMatrix<T, ColMajor> > m_underlying;    
    TransferContainer m_transfers;   ///< Transfer matrices
    OpContainer m_ops;               ///< Operators to be applied in the subspaces
    TransferContainer m_upds;
    mutable std::vector<gsMatrix<T> > m_res, m_p;
};
