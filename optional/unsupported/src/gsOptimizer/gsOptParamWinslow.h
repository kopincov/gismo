/** @file gsOptParamWinslow.h

    @brief Optimization of geometry parameterization using Winslow
 ...
 */
template <typename T>
class gsOptParamWinslow : public gsOptParam<T>
{
public:
    typedef gsOptParam<T> Base;
public:

    gsOptParamWinslow() : gsOptParam<T>() { }

    gsOptParamWinslow(const gsGeometry<T> & geo) : gsOptParam<T>(geo)
    { 
...
public:
    ...
    {
        // Winslow functional (harmonic inverse extension): ||J_c||^2 /det(J)
        this->updateTempDesign(u);

        gsMatrix<T> JacAdj(m_dim,m_dim);
        T val = 0.0;
        ...
        for (; domIt->good(); domIt->next() )
        {
            ...
            {
                // Get Jacobian
                m_geoBasis->jacobianFunc_into(quNodes.col(i), m_tmpCoefs, JacAdj);
                const T detJ = JacAdj.determinant();
                // Adjugate (cofactor transpose)
                JacAdj.adjugateInPlace();
                
                val += quWeights[i] * JacAdj.squaredNorm() / detJ;
            }
        }
    ...
    void gradObj_into( const gsAsConstVector<T> & u, gsAsVector<T> & result) const  
    {
        ...
        // Winslow deriv: 2 * trace( (J*J^T)*(d_cJ*J^T +(d_cJ*J^T)^T ) )
        this->updateTempDesign(u);
            ...
            for ( index_t k = 0; k != numActive; ++k) // For all active
            {
                ...
                if ( m_mapper.is_free_index(kk) ) // interior node?
                {
                    for ( index_t s = 0; s != m_geoDim; ++s) // for all components
                    {
                        ...
                        {
                            const gsMatrix<T> Jaci = Jac.middleCols(i*m_dim,m_dim);
