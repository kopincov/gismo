...
public:
    ...
    gsQualityMeasure(gsGeometry<T>& geom,
    ...
    void optimize(const T orthogonality,
                  const T skewness,
                  ...
                  const bool dumped = false)
    {
        ...
        gsVector<index_t> numNodes(parDim);
...
//                  << std::endl;
        ...
        if (dumped)
        {
            ...
            T startFunc = functional(orthogonality, skewness, eccentricity,
            ...
            for (int i = 0; i != 10; i++)
            {
                ...
                T newFunc = functional(orthogonality, skewness, eccentricity,
                                       uniformity, length, area, intersection,
                                       epsilon);
    ...
    T functional(const T orthogonality,
                 const T skewness,
                 const T eccentricity,
                 const T uniformity,
                 const T length,
...
private:
    ...
    void setMappings()
    {
        ...
        index_t size = mGeom.basis().size();
        index_t matrixSize = size - boundary.rows();
        ...
        index_t boundaryCounter = 0;
        index_t matrixCounter = 0;
        for (index_t basisFun = 0; basisFun != size; basisFun++)
        {
            if (boundaryCounter != boundary.rows() &&
                basisFun == boundary(boundaryCounter, 0))
            {
                boundaryCounter++;
            }
            else
    ...
    /// \brief Assembles matrix \a A and right hand side vector \a b from Jacobian 
    ...
    gsQualityMeasure(const gsQualityMeasure<T>& other);

    // disable assignment operator
    gsQualityMeasure<T>& operator=(const gsQualityMeasure<T>& other);
...
private:
    ...
    gsVector<index_t> mMatrixToBasis;

    /// \brief Maps index of a basis to index of a matrix (row / column).
    gsVector<index_t> mBasisToMatrix;
};
