...
index_t reorderMapperTarget(gsWeightMapper<real_t> &mapper, const std::vector<index_t>& perm, gsPermutationMatrix* permMatrix)
{
...
// assumes row mayor
void copyToBlock (const gsWeightMapper<real_t> &src, gsWeightMapper<real_t>::LToGMatrix &dst, index_t rowShift, index_t colShift)
{
    src.optimize();
    for (index_t r=0; r<src.getNrOfSources();  ++r)
        for (gsWeightMapper<real_t>::Iterator entry=src.fastSourceToTarget(r); entry;  ++entry)
...
{
    gsWeightMapper<real_t>* result=new gsWeightMapper<real_t>();
...
{
    gsWeightMapper<real_t>::LToGMatrix &resultMatrix=result.asMatrix();
