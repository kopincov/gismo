...
struct gsGeoNoTransform
{
    ...
    // stack together b_values in a big vector
                                            ...
                                            )
    {
        ...
        for (int a=0; a<numA; ++a)
        {
            for (int c=0; c<TarDim; ++c)
            {
                ...
                {
                    result.template block<1,ParDim>(a*TarDim+c,p*ParDim) = 
                        b_eval->derivs().template block<ParDim,1>(a*ParDim*TarDim+c*ParDim,p).transpose() ;
                }
            }
        }
    }
                                        ...
                                        )
    {
        ...
        const int secDerSize1D = (ParDim*ParDim+ParDim)/2;
                                        ...
                                        )
    {
        ...
        const int secDerSize1D = AmbDim*(AmbDim+1)/2;
        const int secDerSize   = secDerSize1D*TarDim;
        ...
        for (int a=0; a<numA; ++a)
        {
            for (int c=0; c<TarDim; ++c)
            {
                result.row(a*TarDim+c) = b_eval->derivs2().middleRows(a*secDerSize+c*secDerSize1D,AmbDim).colwise().sum();
            }
        }
        ...
        : public gsGeoNoTransform<T,ParDim,TarDim, AmbDim>
{
    ...
    {
        const index_t numPts=b_values[0].cols();
        const index_t der2PhSize=AmbDim*(AmbDim+1)/2;
        const index_t der2PrSize=ParDim*(ParDim+1)/2;
        ...
        result.setZero(totAct*der2PhSize*TarDim,numPts);

        gsMatrix<T> tmp;
        index_t blockOffSet=0;
        for (index_t c=0; c<TarDim; ++c)
        {
            const index_t der1Shift = activeNum[c];
            const index_t der1Size  = activeNum[c]*ParDim;
            const index_t der2Shift = der1Shift+der1Size;
            const index_t der2Size  = activeNum[c]*der2PrSize;


            for (index_t p=0; p<numPts;++p)
            {
                g_Eval->transformDeriv2Hgrad(p,
                                             b_values[c].middleRows(der1Shift,der1Size),
                                             b_values[c].middleRows(der2Shift,der2Size),
                                             tmp
                                             );
                for (index_t w=0; w<activeNum[c];++w)
                {

                    result.block(der2PhSize*TarDim*w+blockOffSet,
                                 p,
                                 der2PhSize,
                                 1)
                            =tmp.row(w).transpose();
            ...
            blockOffSet+=(activeNum[c])*der2PhSize*TarDim+der2PhSize;
        }
    }
