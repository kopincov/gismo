...
template <typename T>
gsBasisEvaluator<T> * makeBasisEvaluator ( const gsBasis<T> &basis, index_t shift, unsigned flags=0, const gsGeometry<T> *geo=NULL, ValueTransformationType geoTrans=INVERSE_COMPOSITION );
...
template <typename T>
gsBasisEvaluator<T> * makeBasisEvaluator ( const std::vector<gsBasis<T> *> &basis, const std::vector<index_t>& shifts , unsigned flags=0, const gsGeometry<T> *geo=NULL, ValueTransformationType geoTrans=INVERSE_COMPOSITION);
...
public:
    ...
    virtual void       addActiveShift (unsigned shift) = 0;

    /// info functions
    virtual gsDomainIterator<T>* getDomainIterator(const boxSide bId=boundary::none) const {return NULL;}
    virtual gsVector<unsigned>   getDegree()                               const =0;
    virtual std::vector<index_t> getBoundary(boxSide bs,index_t offset=0)  const =0;
    ...
    const          gsMatrix<index_t>        & actives     ()          const {return m_actives;}
...
**/
template <typename T, short_t ParDim, short_t TarDim, typename geoTransformType>
class gsGenericBasisEvaluator
    : public gsBasisEvaluator<T>
{
public:
    gsGenericBasisEvaluator(const gsBasis<T> *(&basis)[TarDim],unsigned flags);
    gsGenericBasisEvaluator(const gsBasis<T> *(&basis)[TarDim],const std::vector<index_t>& shifts,unsigned flags);
private:
    void init (const gsBasis<T> *(&basis)[TarDim],unsigned flags);
public:
    ...
    virtual void addActiveShift (unsigned shift);

    /// info functions
    virtual gsDomainIterator<T>* getDomainIterator(const boxSide bId)   const;
    virtual gsVector<unsigned>   getDegree()                            const;
    virtual std::vector<index_t> getBoundary(boxSide bs,index_t offset) const;

protected:
    int                         m_max_deriv;
    const gsBasis<T>           *m_basis[TarDim];
