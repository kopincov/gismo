...
public:
    ...
    void eval_into(const gsMatrix<T>& u, gsMatrix<T>& result) const
    {
        gsMatrix<T> tmp;
        m_geo.eval_into(u, tmp);
        //m_geo.deriv_into(tmp, result);
        m_f.deriv_into(tmp, result);
        //gsInfo << " gsGradientField: result of deriv_into = \n" << result << "\n";
...
/**
@brief Generates a field with value being the divergence of an isogeometric function.
For a multipatch geometry, use the static method of gismo::gsFieldCreator

\ingroup Core
*/
template<class T>
class gsDivergenceField : public gsFunction<T>
{
public:
    /// Shared pointer for gsDivergenceField
    typedef memory::shared_ptr< gsDivergenceField > Ptr;

    /// Unique pointer for gsDivergenceField
    typedef memory::unique_ptr< gsDivergenceField > uPtr;

    gsDivergenceField( gsGeometry<T> const & geo, gsFunction<T> const & f)
            : m_geo(geo), m_f(f)
    { }

    GISMO_CLONE_FUNCTION(gsDivergenceField)

    void eval_into(const gsMatrix<T>& u, gsMatrix<T>& result) const
    {
        gsMatrix<T> tmp;
        m_f.deriv_into(u,tmp);
        convertValue<T>::derivToDiv(tmp, result, this->info());
    }
    ...
    /// Prints the object as a string.
    std::ostream &print(std::ostream &os) const
    { os << "Divergence field of "; return os; };
private:
    const gsGeometry<T> & m_geo;
    const gsFunction<T> & m_f  ;
};
...
struct gsFieldCreator
{
    ...
    static gsField<T> divergence(gsMultiPatch<T> const & mp, gsFunction<T> const & f)
    {
        gsPiecewiseFunction<T> * nFields = new gsPiecewiseFunction<T>(mp.nPatches());

        for (index_t k=0; k< mp.nPatches(); ++k)
            nFields->addPiecePointer( new gsDivergenceField<T>(mp.patch(k), f) );
