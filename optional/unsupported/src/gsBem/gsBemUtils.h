...
{
public:
    /// Shared pointer for gsGreenFunction2d
    typedef memory::shared_ptr< gsGreenFunction2d > Ptr;

    /// Unique pointer for gsGreenFunction2d
    typedef memory::unique_ptr< gsGreenFunction2d > uPtr;
...
public:
  ...
  GISMO_CLONE_FUNCTION(gsGreenFunction2d)
...
{
public:
    /// Shared pointer for gsBemSolution
    typedef memory::shared_ptr< gsBemSolution > Ptr;

    /// Unique pointer for gsBemSolution
    typedef memory::unique_ptr< gsBemSolution > uPtr;
    ...
    gsBemSolution(gsPlanarDomain<T> * domain, std::vector<gsGeometry<T> *> const & flux,
                  ...
                  bool par_rhs= false)
        : m_pdomain( domain->clone().release() ), m_par_rhs(par_rhs)
    {
        m_flux=flux;
    ...
    GISMO_CLONE_FUNCTION(gsBemSolution)
   ...
   virtual void eval_into(const gsMatrix<T>& u, gsMatrix<T>& result) const
    {
       ...
       // gsVector<T> unormal;
        ...
        for(index_t v=0; v != nloops; v++)
        {
            ...
            all_loops.push_back(  m_pdomain->loop(v).singleCurve().release() );
        }
    ...
    // first time: computing integral over (0,1) and evaluate the solution u
...

            {
               ...
               const std::vector<T> & breaks = m_breaks[v];
                ...
                typename gsGeometryEvaluator<T>::uPtr loop_v_eval(
                    getEvaluator(NEED_VALUE | NEED_JACOBIAN, *(all_loops[v])));
                
                m_green->setSourcePoint( u.col(i) ) ;

                // Start iteration over sub-elements
                for (size_t e=1; e!= breaks.size(); ++e)
                {
...
//  }
    ...
    void deriv_into(const gsMatrix<T>& u, gsMatrix<T>& result) const
    {
        ...
        for(index_t v=0; v< nloops; v++)
        {
            ...
            // Evaluator on the current loop
            typename gsGeometryEvaluator<T>::uPtr loop_v_eval(
                getEvaluator(NEED_VALUE | NEED_GRAD_TRANSFORM, *(all_loops[v])));
...
//                  }
                    ...
                    do
                    {
                        ...
                        if ( math::abs(lVal.at(curr.index) + rVal.at(curr.index) - curr.value) > 1e-3  )
                        {
                            ...
                            res_iv.at(curr.index) += lVal.at(curr.index) + rVal.at(curr.index);
...
//                    result.col(i) += res_iv.col(0);
    ...
    inline T evalOnElement( gsMatrix<T> quNodes,
                            gsVector<T> quWeights,
                            int v,
                           const typename gsGeometryEvaluator<T>::uPtr & loop_v_eval,
...
T gsBemSolution<T>::distanceL2(const gsFunction<T> & f) const
{
    ...
    for(index_t v=0; v< nloops; v++)
    {
        ...
        // Evaluator on the current loop
        typename gsGeometryEvaluator<T>::uPtr loop_v_eval(getEvaluator(NEED_VALUE | NEED_JACOBIAN, *(all_loops[v])));
        ...
        for (size_t e = 1; e != breaks.size(); ++e)
        {
            ...
            f.jacobian_into( loop_v_eval->values(), fderiv );
