...
//                typename gsGeometryEvaluator<T>::uPtr loop_s_eval (
//                    getEvaluator(GEO_NEED_VALUE   |
//                                 GEO_NEED_JACOBIAN, *(all_loops[s])) );
//=======
            // Evaluator on the current loop
            typename gsGeometryEvaluator<T>::uPtr loop_s_eval(getEvaluator(NEED_VALUE | NEED_JACOBIAN, *(all_loops[s])));
        ...
        for(int  v=0; v< nloops; v++)
        {
            ...
            Fluxf.push_back( m_basis[v]->makeGeometry(give(B)).release() );
            numDofs_v += n_v;
        }
