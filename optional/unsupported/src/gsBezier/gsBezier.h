...
template<class T>
class gsBezier :  public gsGeoTraits<1,T>::GeometryBase
{
public:
    ...
    /// Shared pointer for gsBezier
    typedef memory::shared_ptr< gsBezier > Ptr;

    /// Unique pointer for gsBezier
    typedef memory::unique_ptr< gsBezier > uPtr;

public:
    
    /// Construct B-Spline by basis functions and coefficient matrix
    gsBezier( const Basis & basis, gsMatrix<T> coefs ) :
    Base( basis, give(coefs)) { }
    
    /// Construct Bezier curve by degree, domain and coefficient matrix
    gsBezier( const unsigned p, gsMatrix<T> coefs, const T & u0=0, const T & u1= 1)
    {
