 ...
 */
template<typename T>
class gsIETIJumpOperator
{

    typedef typename gsIETIAssembler<T>::patchDof patchDof; //patch + local number
    ...
    std::vector<std::vector<T> > scalings;

    /// \brief  Stores for each patch the coefficient for scaling strategy
    std::vector<T> m_coeffs;

public:
    gsIETIJumpOperator(const gsIETIAssembler<T>& ass, IETIPrecondScaling::strategy sc = IETIPrecondScaling::none);
     ...
     */
    void applyTil(const gsMatrix<T>& xP, const std::vector<gsMatrix<T> > & x2, gsMatrix<T> & result) const;
 ...
 */
template<class T>
class gsIETISolver : public gsLinearOperator<>
{
public:
    
    /// Shared pointer for gsIETISolver
    typedef memory::shared_ptr<gsIETISolver> Ptr;

    /// Unique pointer for gsIETISolver
    typedef memory::unique_ptr<gsIETISolver> uPtr;
...
protected:
     ...
     */
    void applyMinEnergy(const gsMatrix<T> & inputP, const std::vector<gsMatrix<T> > & inputD, gsMatrix<T> & resultP, std::vector<gsMatrix<T> > & resultD) const;
     ...
     */
    void applyStdIETI(const gsMatrix<T> & inputP, const std::vector<gsMatrix<T> > & inputR, gsMatrix<T> & resultP, std::vector<gsMatrix<T> > & resultR) const;
...
public:
     ...
     */
    gsIETISolver(gsIETIAssembler<T>& assembler): m_B(assembler), m_ass(assembler), info(assembler.getInfo())
    {
        m_rhs.resize(info.lagrangeMult,info.nRhs);
    }

    static Ptr make(gsIETIAssembler<T>& assembler)
    { return memory::make_shared( new gsIETISolver<T>( assembler ) ); }
    ...
    void init()
    {
        ...
        if(!m_ass.getOptions().opt.getSwitch("NoMinimumEnergy")){
            //process rhs
            applyMinEnergy(rhs_p,rhs_r,rhs_p_t,rhs_r_t);
        }
        else
        {
            ...
            applyStdIETI(rhs_p,rhs_r,rhs_p_t,rhs_r_t);
        }
     ...
     */
    index_t rows() const {return info.lagrangeMult;}

    /**
     * @brief cols returns the columns of the matrix. Inherited from base class
     * @return number of columns
     */
    index_t cols() const {return info.lagrangeMult;}
     ...
     * @param resR The dual/remaining part of the solution
     *
     */
...
namespace internal
{
template<typename T> class gsIETI_BWrapper;
template<typename T> class gsIETI_BTWrapper;
template<typename T> class gsIETI_KWrapper;
}

/**
 * \brief This class represents the application of the IETI system matrix and is
 * designed to fit as input for the gsMinimalResidual class. This means that this class is inherited from gsLinearOperator<>. It also returns the
 * corresponding right hand side.
 */
template<class T>
class gsInexactIETISolver : public gsLinearOperator<>
{
public:

    /// Shared pointer for gsIETISolver
    typedef memory::shared_ptr<gsInexactIETISolver> Ptr;

    /// Unique pointer for gsIETISolver
    typedef memory::unique_ptr<gsInexactIETISolver> uPtr;
    ...
    friend class internal::gsIETI_BWrapper<T>;
    friend class internal::gsIETI_BTWrapper<T>;
    friend class internal::gsIETI_KWrapper<T>;
protected:
    /**
     * @brief applyMinEnergy This funciton performs the application of the inverse Schur complement S
     * of the stiffness matrix via the minimal energy algorithm. It is part of the apply(..) function.
     ...
     * @param resultD The dual part of the result  For each patch (dofsB x nRhs)
     */
    void applyMinEnergy(const gsMatrix<T>& input, gsMatrix<T>& output)const;
     ...
     * @param resultR The remaining part of the result  For each patch (dofsB x nRhs)
     */
    void applyStdIETI(const gsMatrix<T>& input, gsMatrix<T>& output)const;
...
public:
     ...
     *  the jump operator B (gsIETIJumpOperator) and computes the appropriate rhs.
     */
    gsInexactIETISolver(gsIETIAssembler<T>& assembler);

    static Ptr make(gsIETIAssembler<T>& assembler)
    { return memory::make_shared( new gsInexactIETISolver( assembler ) ); }
     ...
     * @param solVec the solution in matrix form
     */
    void calculateSolution(const gsMatrix<T> &x, gsMatrix<T>& solVec) const
    {
        gsMatrix<T> x_temp = x;
        typename gsMatrix<T>::BlockView inp_block = x_temp.blockView(m_rowBlocksFull,m_colBlocks);
     //   gsInfo<<"uP: "<<inp_block(2*info.numberPatches)<<"\n";

        m_ass.processSolution(inp_block,solVec);
    }
    ...
    typename gsBlockOp<T>::Ptr makeBlockOp()
    {
        gsBlockOp<T>* blockOp = new gsBlockOp<T>(2,2);
        blockOp->addOperator(0,0,internal::gsIETI_KWrapper<T>::make(*this));
        blockOp->addOperator(1,0,internal::gsIETI_BWrapper<T>::make(*this));
        blockOp->addOperator(0,1,internal::gsIETI_BTWrapper<T>::make(*this));
        return memory::make_shared(blockOp);
    }
...
protected:
    ...
    /// \brief for each patch the matrix Krp
    std::vector<gsSparseMatrix<T> > m_Krp;

    gsVector<index_t> m_rowBlocks;
    gsVector<index_t> m_rowBlocksFull;
...
{
template<typename T>
class gsIETI_BWrapper : public gsLinearOperator<T>
{
public:
    /// Shared pointer for gsIETISolver
    typedef memory::shared_ptr<gsIETI_BWrapper<T> > Ptr;

    /// Unique pointer for gsIETISolver
    typedef memory::unique_ptr<gsIETI_BWrapper<T> > uPtr;
    ...
    typedef real_t RealScalar;
public:
    gsIETI_BWrapper(const gsInexactIETISolver<T>& iIETI) :m_iIETI(iIETI) {}

    static Ptr make(const gsInexactIETISolver<T>& iIETI)
    { return memory::make_shared( new gsIETI_BWrapper<T>(iIETI) ); }

    void apply(const gsMatrix<T> &input, gsMatrix<T> &x) const
    {
        gsMatrix<T>& inp = const_cast<gsMatrix<T>&>(input);
        typename gsMatrix<T>::BlockView inp_block = inp.blockView(m_iIETI.m_rowBlocks.head(2*m_iIETI.info.numberPatches+1),m_iIETI.m_colBlocks);
        std::vector<gsMatrix<T> > u(m_iIETI.info.numberPatches);
        for(size_t np=0; np<m_iIETI.info.numberPatches;++np)
        {
            u[np].resize(m_iIETI.info.dofsB[np]+m_iIETI.info.dofsI[np],input.cols());
            u[np].topRows(m_iIETI.info.dofsB[np])=inp_block(2*np);
            u[np].bottomRows(m_iIETI.info.dofsI[np])=inp_block(2*np+1);
        }
        m_iIETI.m_B.applyTil(inp_block(2*m_iIETI.info.numberPatches), u, x);

    }

    index_t rows() const { return m_iIETI.info.lagrangeMult;}
    index_t cols() const { return m_iIETI.m_ass.systemSize()-rows();}
private:
   const gsInexactIETISolver<T>& m_iIETI;

};

template<typename T>
class gsIETI_BTWrapper : public gsLinearOperator<T>
{
public:
    /// Shared pointer for gsIETISolver
    typedef memory::shared_ptr<gsIETI_BTWrapper<T> > Ptr;

    /// Unique pointer for gsIETISolver
    typedef memory::unique_ptr<gsIETI_BTWrapper<T> > uPtr;
    ...
    typedef real_t RealScalar;
public:
    gsIETI_BTWrapper(const gsInexactIETISolver<T>& iIETI) :m_iIETI(iIETI)
    {
        x1P.setZero(m_iIETI.info.dofTotalP,m_iIETI.info.nRhs);
        x1B.resize(m_iIETI.info.numberPatches);
        m_rowBlocks = m_iIETI.m_rowBlocks.head(2*m_iIETI.info.numberPatches+1);
    }

    static Ptr make(const gsInexactIETISolver<T>& iIETI)
    { return memory::make_shared( new gsIETI_BTWrapper<T>( iIETI) ); }

    void apply(const gsMatrix<T> &input, gsMatrix<T> &x) const
    {
        x.setZero(rows(),input.cols());
        x1P.setZero(m_iIETI.info.dofTotalP,input.cols());
        gsVector<index_t> colBlocks(1);
        colBlocks<<input.cols();

        m_iIETI.m_B.applyTransTil(input, x1P, x1B);

        typename gsMatrix<T>::BlockView out_block = x.blockView(m_rowBlocks,colBlocks);

        out_block(2*m_iIETI.info.numberPatches) += x1P;
        for(size_t np=0; np<m_iIETI.info.numberPatches;++np)
            out_block(2*np) += x1B[np];
    }

    index_t cols() const { return m_iIETI.info.lagrangeMult;}
    index_t rows() const { return m_iIETI.m_ass.systemSize()-cols();}
private:
   const gsInexactIETISolver<T>& m_iIETI;

   gsVector<index_t> m_rowBlocks;

   mutable gsMatrix<T> x1P;
   mutable std::vector<gsMatrix<T> > x1B;

};

template<typename T>
class gsIETI_KWrapper : public gsLinearOperator<T>
{
public:
    /// Shared pointer for gsIETISolver
    typedef memory::shared_ptr<gsIETI_KWrapper<T> > Ptr;

    /// Unique pointer for gsIETISolver
    typedef memory::unique_ptr<gsIETI_KWrapper<T> > uPtr;
    ...
    typedef real_t RealScalar;
public:
    gsIETI_KWrapper(const gsInexactIETISolver<T>& iIETI) :m_iIETI(iIETI)
    {
        m_rowBlocks = m_iIETI.m_rowBlocksFull.head(m_iIETI.info.numberPatches+1);
    }

    static Ptr make(const gsInexactIETISolver<T>& iIETI)
    { return memory::make_shared( new gsIETI_KWrapper<T>( iIETI) ); }

    void apply(const gsMatrix<T>& input, gsMatrix<T>& output)const
    {
        int nRhs = input.cols();
        output.setZero(rows(),nRhs);
        Eigen::setNbThreads(1);
        gsVector<index_t,1> colBlocks;
        colBlocks<<input.cols();
        gsMatrix<T>& inp = const_cast<gsMatrix<T>&>(input);
        typename gsMatrix<T>::BlockView inp_block = inp.blockView(m_rowBlocks,colBlocks);
        typename gsMatrix<T>::BlockView out_block = output.blockView(m_rowBlocks,colBlocks);

    #pragma omp parallel
        {
            gsMatrix<T> sol,dummy;
            dummy.setZero(m_iIETI.info.dofTotalP,nRhs);
    #pragma omp for schedule(static, 1) nowait
            for(size_t k = 0; k<m_iIETI.info.numberPatches; k++)
            {
                sol.setZero(m_iIETI.info.dofsB[k]+m_iIETI.info.dofsI[k], nRhs);
                m_iIETI.m_ass.applyK(k,inp_block(k),sol);

           //     m_ass.embeddingTrans(sol,k,dummy,sol);
    ...
    #pragma omp single nowait
            {
                gsMatrix<T> resultP;
                if(m_iIETI.info.dofTotalP != 0)
                    m_iIETI.m_ass.applySpp(inp_block(m_iIETI.info.numberPatches),resultP);
                out_block(m_iIETI.info.numberPatches)= resultP;
            }
        }
        Eigen::setNbThreads(0);
    }

    index_t cols() const { return m_iIETI.m_ass.systemSize() - m_iIETI.info.lagrangeMult;}
    index_t rows() const { return cols();}
private:
    const gsInexactIETISolver<T>& m_iIETI;
...
#include GISMO_HPP_HEADER(gsIETISolver.hpp)
#endif
