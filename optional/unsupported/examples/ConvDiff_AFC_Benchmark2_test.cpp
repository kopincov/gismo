...
int main(int argc, char *argv[])
{
    ...
    gsFunctionExpr<real_t>  coeff_b("1","1", 2);
    ...
    gsFunctionExpr<real_t>  coeff_bK("1","1", 2);
	...
	gsMultiPatch<real_t> mp( *gsNurbsCreator<>::BSplineRectangle(-1.0,0.0,0.0,1.0) );
	gsBoundaryConditions<> BCs;
	//for (gsMultiPatch<>::const_biterator
	//	bit = mp.bBegin(); bit != mp.bEnd(); ++bit)
	//{
    BCs.addCondition(1, condition_type::dirichlet, &g);
    BCs.addCondition(3, condition_type::dirichlet, &g);
	//}
	//
	...
	//gsField<> * sol2h = NULL;
	gsField<> solh;
	//gsField<> * sol4hSUPG = NULL;
	//gsField<> * sol2hSUPG = NULL;
	gsField<> solhSUPG;
		...
		if ( galerkin5.numDofs() )
		{
            ...
            for (int k=1; k<=4;k++)
            {
                ...
                for (int l = 0; l<bases[0].component(boxSide(k).direction()).size();l++)
                {
                    if (k==3)
                    {
                        if (l<bases[0].component(boxSide(k).direction()).size())
                        {
                ...
                for (int e=0; e<K.outerSize(); ++e)
                    for (gsSparseMatrix<real_t>::InnerIterator it(galerkinK.matrix(),e); it; ++it)
                    {
                        ...
                        if (l > k || (l<k && galerkinK.matrix().at(l,k)==0)) //make sure that it still works if the matrix is not symmetric (sparsity)
                        {
                            ...
                            f.coeffRef(k,l) = temp;
                            Pp.coeffRef(k) += math::max((real_t)0,temp);
                            Pm.coeffRef(k) += math::min((real_t)0,temp);
                            Qp.coeffRef(k) += math::max((real_t)0,-temp);
                            Qm.coeffRef(k) += math::min((real_t)0,-temp);
                            Qp.coeffRef(l) += math::max((real_t)0,temp);
                            Qm.coeffRef(l) += math::min((real_t)0,temp);
                        }
                        else
                        {
                            f.coeffRef(k,l) = 0.0;
