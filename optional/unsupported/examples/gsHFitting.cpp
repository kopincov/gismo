/** gsHFitting.cpp
 *
 * For testing of the class gsHFitting.
 *
 ...
 *
 * Author(s): Gabor Kiss, Dominik Mokris (dominik.mokris@jku.at).
 */
...
bool checkResults( gsMatrix<> results )
{
    ...
    if( ( results(0,2) > 6.0e-5 ) || (results(1,2) > 1.5e-5 ) || (results(2,2) > 1.8e-5 ) || (results(3,2) > 2e-5) )
    {
        gsWarn << "Minimum errors seem to be incorrect.\n";
        result = false;
    }
    if( ( results(0,3) > 3.2e-1 ) || (results(1,3) > 4.5e-4 ) || (results(2,3) > 3.8e-4) || (results(3,3) > 3.7e-4) )
    {
    ...
    if( ( results(0,5) > 1.3 ) || (results(1,5) > 2.8e-6 ) || (results(2,5) > 2.3e-6 ) || (results(3,5) > 2.2e-6) )
    {
...
int main(int argc, char *argv[])
{
    ...
    gsHFitting<2, real_t> ref(inputParams, inputValues , THB, 0.01, ext, lambda);
