/** @file spacetime_adaptive.cpp

    @brief Example for using the gsSpaceTime with adaptive refinement with THB-splines.
...
#include <gsAssembler/gsErrEstSpaceTimeResidual.h>
...
int main(int argc, char *argv[])
{
  ...
  gsFunctionExpr<>  f("pi*sin(pi*x)*(cos(pi*y) + pi*sin(pi*y))",2);
  //gsFunctionExpr<>  f("if( (x-0.25)^2 + (y-1.6)^2 < 0.2^2, 1, 0 )",2);
  gsFunctionExpr<>  g("sin(pi*x)*sin(pi*y)",2);
  gsMultiPatch<> patches( *gsNurbsCreator<>::BSplineRectangle(0.0,0.0,1.0,1.0) );
  ...
  */
  ...
  gsFunctionExpr<real_t>  theta("1", 2);
  ...
  for( int RefineLoop = 1; RefineLoop <= RefineLoopMax ; RefineLoop++ )
  {
      ...
      gsErrEstSpaceTimeResidual<real_t> errEst(sol,f);
