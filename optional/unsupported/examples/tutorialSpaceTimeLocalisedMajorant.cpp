/** @file tutorialSpaceTimeLocalisedMajorant.cpp
    ...
    (locally stabilised version of the scheme in gsSpaceTimeSolver)
    with adaptive refinement with THB-splines using functional error
    estimates for the error control and following form them indicator
    as a refinement criteria.
...
#include <gsErrorEstimates/gsTestSpaceTimeLocalisedMajorant.h>  // Class that tests majorant

// Classes targeted to the locally stabilised scheme and localised norms
#include <gsErrorEstimates/gsSpaceTimeLocalisedAssembler.h>
#include <gsErrorEstimates/gsSpaceTimeLocalisedNorm.h>          // (sum_K || grad_x w ||^2_K + h_K || w_t ||^2_{K})^{1/2}
#include <gsErrorEstimates/gsSpaceTimeLocalisedSigmaTNorm.h>    // (sum_K || w ||^2_{\Gamma_{K,T}} + h_K || grad_x w ||^2_{\Gamma_{K,T}})^{1/2}
#include <gsErrorEstimates/gsSpaceTimeLocalisedDtNorm.h>        // (sum_K delta^2_K * || \partial_t e ||^2_K)^{1/2}
#include <gsErrorEstimates/gsSpaceTimeLocalisedDeltaxNorm.h>   // (sum_K delta^2_K * || Delta_x u ||^2_K)^{1/2}
...
int main(int argc, char *argv[])
{
    ...
    bool isAdaptive     = true;             // Flag if the adaptive refinement mode is on
    bool withMajorant   = false;            // Flag if the error control if driven by the majorant
    bool withResidualEstimator = true;      // Flag if the error control is driven by the residual error estimator
    ...
    // exampleNumber(41), d(2);         // 41 example: 2d [0, 1] x (0, 2), u = sin(pi*x)*abs(1 - y)^(2/3)
    // exampleNumber(42), d(2);         // 42 example: 2d [0, 1] x (0, 2), u = sin(pi*x)*abs(1 - y) with C^0 continuaty
    // exampleNumber(43), d(2);         // 43 example: 2d [0, 1] x (0, 2), u = x^(5/2)(1-x)*t^(3/4)
    exampleNumber(44), d(2);         // 44 example: 2d moving spatial domain, u = x^(5/2)(1-x)*t^(3/4)
    ...
    unsigned int numInitUniformRefV(1), numInitUniformRefY(1), numInitUniformRefW(1), numTotalAdaptRef(9);
    ...
    gsVector<real_t> eL2Vector(numTotalAdaptRef),           // || e_i ||_Q          := || u - u_i ||_Q
            ...
            residualVector(numTotalAdaptRef),               // \sum_K h^2_K || Delta_x u_i + f - (u_i)_t ||^2_K
    ...
    gsVector<real_t> gradxeTVector(numTotalAdaptRef),
            ...
            majIIVector(numTotalAdaptRef),
            majIIGapVector(numTotalAdaptRef),
            eWL2Vector(numTotalAdaptRef), eWH1Vector(numTotalAdaptRef);
    ...
    gsVector<double> timeAsmbV(numTotalAdaptRef),
            ...
            timeAsmbSpaceTimeResidual(numTotalAdaptRef),
    ...
    for( unsigned int refCount = 0; refCount < numTotalAdaptRef ; refCount++ )
    {
        ...
        std::vector<real_t> mdDistr, majDistr, mIIdDistr, eH1Distr, eL2Distr, eH2Distr,
                ...
                minDistr, eIdentDistr, residualDistr;
        ...
        residualDistr.resize(elemNumber);
...
#pragma omp section
            {
                gsSpaceTimeLocalisedDeltaxNorm<real_t> eSpaceTimeDeltaxDeltaK(v, * uForOMP[15]);
...
#pragma omp section
            {
                gsSpaceTimeLocalisedDtNorm<real_t> eSpaceTimeDtDeltaK(v, * uForOMP[16]);
...
#pragma omp section
            {
                gsErrEstSpaceTimeResidual<real_t> residualEstimator(v, * fForOMP[1]);
                testSpaceTime.gsCalculateDistribution(residualEstimator, residualDistr, elemNumber,
                                                      residualVector, timeAsmbSpaceTimeResidual, refCount);
                gsInfo << "t_{e/w} ( sum_K eta^2_K )   = " <<  timeAsmbSpaceTimeResidual[refCount] << " sec.\n";
            }
#pragma omp section
            {
...
#pragma omp section
        ...
        eFullSpaceTimeLocVector[refCount]       = math::sqrt(math::pow(eSpaceTimeSpaceGradVector[refCount], 2)
                                                             + math::pow(eSpaceTimeDtDeltaKVector[refCount], 2)
                                                             + 0.5 * math::pow(eTVector[refCount], 2));
        ...
        testSpaceTime.gsLogRefinementIterationErrorReport(refCount, hmaxVector, hminVector,
                                                          ...
                                                          eIdentVector, eFullIdentVector, residualVector,
        ...
        if (refCount < numTotalAdaptRef - 1) {
            testSpaceTime.gsExecuteRefinement(spaceTimeAssemblerV,
                                              ...
                                              basisYVector, basisWVector, mdDistr, //mIIdDistr, // eIdentDistr, //eSpaceTimeSolOperDistr, //eH1Distr, //mdDistr, //eSpaceTimeDistr, //eSpaceTimeSolOperDistr, //eH1Distr, //eIdentDistr, //mdDistr,
                                              adaptRefCrit,
                                              markingParamTheta,
                                              refCount,
                                              yBasisRefDelay,
