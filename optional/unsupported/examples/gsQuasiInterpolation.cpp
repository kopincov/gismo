...
template<typename T>
T computeError(const gsFunction<T> &fun, const gsBSpline<T> &spl, const int &numVals)
{
    T a = spl.basis().knot(0);
    T b = spl.basis().knot(spl.basis().knots().size()-1);
    gsMatrix<T> vals(1,numVals+1);
    for(int i=0; i<=numVals; i++)
        vals(i) = a+(b-a)*i/numVals;
...
template<typename T>
bool errorAnalysis(const gsFunction<T> &fun, gsBSplineBasis<T> basis, int type, int numRef)       //ToDo: Ratio-test, return true or false..
{
    gsBSpline<T> result;
    ...
    for(int i=0; i<numRef; i++)
    {
        switch(type)
        {
        case(1): gsQuasiInterpolate<T>::Schoenberg(basis, fun, result); expConvRate = 2.0; break;
        case(2): gsQuasiInterpolate<T>::Taylor(basis, fun, deg, result); expConvRate = (deg+1); break;
        case(3): gsQuasiInterpolate<T>::EvalBased(basis, fun, true, result); expConvRate = (deg+1); break;
        case(0): gsQuasiInterpolate<T>::EvalBased(basis, fun, false, result); expConvRate = (deg+1); break;
        ...
        {
            ratio = prevError/error;
            gsInfo<<",  err_"<<i-1<<"/err_"<<i<<" = "<< ratio <<"\n";
        }
        prevError = error;
        //*/
        basis.uniformRefine();
...
bool polyReconstructionTaylor(const gsFunction<T> &fun, gsBSplineBasis<T> basis, int degPoly)
{
    ...
    gsQuasiInterpolate<T>::Taylor(basis, fun, deg, result);
...
public:
    ...
    void eval_into(const gsMatrix<T> &u, gsMatrix<T> &result) const
    {
        ...
        for(int i=0; i<u.size(); i++)
        {
            T valX = math::cos(2*EIGEN_PI*u(i));
            T valY = math::sin(2*EIGEN_PI*u(i));
            result(0,i) = valX;
            result(1,i) = valY;
        }
    }
    ...
    void evalAllDers_into(const gsMatrix<T> &u, int n, std::vector<gsMatrix<T> > &result) const
    {
        ...
        {
            sincos(0,i) = 
            cossin(1,i) = math::cos(2*EIGEN_PI*u(i));
            sincos(1,i) =
            cossin(0,i) = math::sin(2*EIGEN_PI*u(i));
...
//----------------- testing --------------------
    ...
    gsFunctionExpr<> myPolyQuad("-5*x^2 + 22x - 4",1);
