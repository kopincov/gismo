...
int main(int argc, char *argv[])
{
    ...
    for (index_t r=0; r<=numRefine; ++r)
    {
        ...
        //gsInfo << "\n A = " << gsMatrix<>(A.matrix()).format(HeavyFmt) << std::endl;
        //gsInfo << "\n f = " << gsMatrix<>(A.rhs()).format(HeavyFmt) << std::endl;

        // trilinos::SparseMatrix t_A(A.matrix());
        // trilinos::Vector       t_b(A.rhs(), t_A);
        // trilinos::solver::AmesosSolver t_solver(t_A, trilinos::solver::AmesosSolvers::KLU);
        // //gsInfo << t_solver.currentParams();

        // t_solver.solve(t_b);
        // t_solver.getSolution((gsVector<real_t>&)solVector);
