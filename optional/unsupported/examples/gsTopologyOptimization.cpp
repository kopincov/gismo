...
std::pair<std::pair<int,int>,real_t> findBestGeom(std::vector<gsMatrix<real_t> >& boundaryMatrices,std::vector<unsigned>& sides,
                                          unsigned minFaces,unsigned maxFaces,std::string name,
                                          gsQualityMeasureWeights* givenWeights = NULL,bool print = false)
{
    ...
    for(unsigned i = 0;i<mps.size();++i)
    {
        for(unsigned j = 0;j<mps[i].size();++j)
        {
            ...
            if(!valids[i][j])
                continue;
            toposConsidered++;
            calculateStartingSol(mps[i][j]);
            if(j==156)
            {
                std::string filename=name+"_8_"+util::to_string(j)+"_start3";
                gsWriteParaview(mps[i][j],filename,1000,false,false);
            }
            std::vector<patchCorner> corners;
            std::vector<patchSide> bounds = mps[i][j].boundaries();
            for(unsigned k = 0;k<bounds.size();++k)
            {
                std::vector<boxCorner> boxCorners;
                bounds[k].side().getContainedCorners(2,boxCorners);
                for(unsigned l=0;l<boxCorners.size();++l)
                {
                    patchCorner c(bounds[k].patch,boxCorners[l]);
                    corners.push_back(c);
                }
            }
            gsCompositeIncrSmoothnessGeom<2,real_t> compGeom(mps[i][j],corners,0);
            ...
            if(print)
            {
                ...
                if(regular)
                {
                    gsWriteParaview(mp,filename,1000,false,false);
...
void scaleToSquare(std::vector<gsMatrix<real_t> >& boundaryMatrices,real_t size=1)
{
    ...
    real_t xval,yval;
    for(unsigned i = 0;i<boundaryMatrices.size();++i)
    {
        for(int j = 0;j<boundaryMatrices[i].rows();++j)
        {
...
void comparisonTripplet()
{
    ...
    for(unsigned cases = 1;cases<=3;++cases)
    {
        ...
        gsStopwatch time;
        std::pair<std::pair<int,int>,real_t> res = findBestGeom(boundaryMatrices,sides,1, maxFaces,desc,&weights,false);
...
void comparisonSlightChangeOfGeometry()
{
    ...
    for(unsigned cases = 1;cases<=2;++cases)
    {
        ...
        weights.m_weightArea=false;
        weights.m_approxPoints=0;
        std::pair<std::pair<int,int>,real_t> res = findBestGeom(boundaryMatrices,sides,1, maxFaces,desc,&weights,true);
        descs.push_back(desc);
        mins_nr.push_back(res.first.second);
        mins_patch.push_back(res.first.first);
        mins_val.push_back(res.second);
...
void examples()
{
    ...
    for(unsigned cases = 1;cases<=2;++cases)
    {
        ...
        case 6: // puzzle Piece example outward
        {
            ...
            weights.m_skewness=0.05;
            weights.m_length=0.05;
            //weights.m_uniformity=0.05;
            weights.m_area=5; //1;
