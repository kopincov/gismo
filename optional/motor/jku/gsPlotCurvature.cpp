/** @file gsPlotCurvature

    @brief Plots normal curvatures of the input curve.
...
int main(int argc, char *argv[])
{
    ...
    std::string outputFile("curvatures.txt");
    int numParams = 100;

    gsCmdLine cmd("Plotting the normal curvatures of the curve.");
    cmd.addString("f", "file", "Name of input file", inputFile);
    cmd.addString("o", "output", "Name of output file", outputFile);
    cmd.addInt("n", "num", "Number of parameters.", numParams);
    ...
    writeToTxtFile(parameters, curvs, outputFile);
