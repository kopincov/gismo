/** @file tutorial_parameterization_fitting.cpp
    ...
    @brief Constructs a patch (surface) from discrete sets of
    parameters and points sampled on the boundaries (curves)
...
{
    std::string boundary_data = MOTOR_DATA_DIR "jku/wavy_boundary_data.xml";
    std::string output = "parameterization";
    int num_internal_knots = 5;
    int degree = 2;
    double lambda = 9e-14;

    gsCmdLine cmd("Constructs a patch (surface) from discrete sets of parameters and points sampled on the boundaries (curves) (see gsModelling/gsFitting)");
