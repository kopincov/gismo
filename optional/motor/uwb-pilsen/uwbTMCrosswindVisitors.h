...
public:
    uwbTMCrosswindVisitorKOmega(std::vector< gsDofMapper > & dofMappers, const T viscosity) :
        Base(dofMappers, viscosity), m_deg(0), m_tauStabType(3), m_crosswindType(0), m_cwResidualType(0),
        m_timeStep(0.),
        m_bOldSolSet(false)
    {
        ...
        gsMatrix<T>            & quNodes)
    {
        ...
        m_pStabEvaluator->setCrosswindVars(m_crosswindType, m_deg, m_timeStep, m_solKOmegaGrads, residual);
    ...
    void setCrosswind(const int crosswindType, const int cwResidualType, const int tauType = 3, bool unsteady = false,
                      const T timeStep = 0.)
    {
        m_crosswindType = crosswindType;
