...
template<class T>
class uwbINSSolverDecoupled1 : public uwbINSSolverDecoupledBase<T>
{
...
public:
    uwbINSSolverDecoupled1(uwbINSSolverParams<T>& params) : Base(params)
    {
        //create assembler
        m_pAssembler = new uwbINSAssemblerDecoupled1<T>(params);

        initMembers();
    }

    virtual ~uwbINSSolverDecoupled1()
    {
    }
...
public:
    virtual void initialize()
    {
        ...
        #pragma omp section
        {
            m_solver3.analyzePattern(getAssembler()->matrix3().block(0, 0, udofs, udofs));
            m_solver3.factorize(getAssembler()->matrix3().block(0, 0, udofs, udofs));
        }
        }
    }
    ...
    virtual void nextIteration()
    {
        ...
        #pragma omp parallel for          
        for (int s = 0; s < getAssembler()->getTarDim(); s++) {
            m_solution3.middleRows(s * udofs, udofs) = m_solver3.solve(getAssembler()->rhs3().middleRows(s * udofs, udofs));
        }
    ...
    virtual uwbINSAssemblerDecoupled1<T>* getAssembler()
    {
        return dynamic_cast<uwbINSAssemblerDecoupled1<T>*>(m_pAssembler);
    }
