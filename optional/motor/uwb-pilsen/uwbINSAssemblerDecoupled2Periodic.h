/** @file uwbINSAssemblerDecoupled2Periodic.h
...
template<class T>
class uwbINSAssemblerDecoupled2Periodic : public uwbINSAssemblerDecoupled2<T>, public uwbINSAssemblerPeriodicDecoupledBase<T>
{

public:
    typedef uwbINSAssemblerDecoupled2<T> Base;
    typedef uwbINSAssemblerPeriodicDecoupledBase<T> BasePeriodic;

public:

    uwbINSAssemblerDecoupled2Periodic(uwbINSSolverParams<T>& params) : Base(params), BasePeriodic(params, m_blockAssembler)
    {
        initMembers();
    }

    virtual ~uwbINSAssemblerDecoupled2Periodic()
    {
    }
...
public:
    ...
    virtual void initialize()
    {
        ...
        this->applyPeriodicConditionsOnPressure_into(m_matrix3, m_rhs3, m_perMatrix3, m_perRhs3, true);
    }
...
public:
    ...
    virtual void update(const gsMatrix<T> & solVector, const gsMatrix<T> & solVector2) {
        ...
        m_perSolution2 = solVector2;
        gsMatrix<T> nonPerSolution2;
        this->per2nonperPressure_into(m_perSolution2, nonPerSolution2);

        Base::update(nonPerSolution, nonPerSolution2);
    ...
    virtual void updateRhs3(const gsMatrix<T> & solVector1, const gsMatrix<T> & solVector2)
    {
        ...
        Base::updateRhs3(nonPerSolVector1, nonPerSolVector2);

        this->nonper2perPressure_into(m_rhs3, m_perRhs3);
    }
    ...
    virtual void createSolVector_into(const gsMatrix<T> & solVector1, const gsMatrix<T> & solVector2, const gsMatrix<T> & solVector3, gsMatrix<T> & result) const
    {
        ...
        result.middleRows(0, this->getPShiftPer()) = solVector1;

        result.middleRows(this->getPShiftPer(), this->getPDofsPer()) = solVector3;
    }
    ...
    {
        return BasePeriodic::constructSolution(perSolVector, unk, patchNumbers, sides, relative);
    }
...
protected:
    ...
    using uwbINSAssemblerDecoupled1<T>::m_matrix1;
    using uwbINSAssemblerDecoupled1<T>::m_matrix2;
    using uwbINSAssemblerDecoupled1<T>::m_rhs1;
    using uwbINSAssemblerDecoupled1<T>::m_rhs2;

    // members from uwbINSAssemblerDecoupled2
...
}; //class uwbINSAssemblerDecoupled2Periodic
